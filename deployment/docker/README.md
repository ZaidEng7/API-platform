...
- **Grafana** — `http://localhost:3000` (`admin`/`admin`, dev-only; anonymous viewer access also enabled for convenience). Prometheus datasource is pre-provisioned (fixed `uid: prometheus`, so dashboard JSON can reference it deterministically) — no manual setup needed. Two dashboards are pre-provisioned under the "Platform" folder (`grafana/provisioning/dashboards/json/`):
  - **Phase 5 Service SLOs** (`phase-5-service-slo.json`) — a `$service` dropdown switches between the nine business services, with panels for availability, error rate, and p95/p99 latency against guide §27's own NFR targets (≥ 99.9% monthly, p95 < 500ms, p99 < 800ms).
  - **API Analytics** (`api-analytics.json`, Phase 7) — cross-service view: top 10 endpoints by request rate, top 10 endpoints by error rate, and request/error rate broken down by service. No new instrumentation — built entirely on the same `http_server_requests_seconds_*` metrics the SLO dashboard already uses.

  Both require `management.metrics.distribution.percentiles-histogram.http.server.requests=true` on each service (already set) so Prometheus actually exports the histogram buckets `histogram_quantile()` needs — without it, only average latency would be available, not real percentiles.
