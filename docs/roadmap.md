...
- [ ] Contract tests (Pact) for each adapter — none of the three adapters have one yet; same gap as `contracts/customer-consumer-example` — there's no real consumer for any of them yet either, so a real Pact contract would be asserting made-up behavior. Revisit once a real consumer depends on one.
- [ ] **Exit criteria:** at least one real consumer using an adapter through the Gateway in production — **blocked**: all three adapters are templates against stub backends, not real integrations (Phase 1 Assessment was deferred), so there's no real consumer yet to satisfy this
